...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!fechaInicio || !fechaFin) {
    return NextResponse.json({ error: "Falta fecha_inicio o fecha_fin" }, { status: 400 });
  }

  const { data: profesional, error } = await supabase.from("profesionales").select("*").eq("id", id).single();
  if (error || !profesional) return NextResponse.json({ error: "Profesional no encontrado" }, { status: 404 });
