...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; membresiaId: string }> }) {
  ...
  const { error } = await admin.from("membresias_cuenta").update({ rol }).eq("id", membresiaId).eq("cuenta_id", id);
