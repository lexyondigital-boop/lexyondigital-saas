...
export function AgentesVozView({ permisos, role }: { permisos: Record<string, boolean>; role: "super_admin" | "admin" | "agente" }) {
  ...
  return (
    <div>
      ...
      {categoria === null && role === "super_admin" && <ReporteRetellGlobal />}
    </div>
  );
}
...
// sub-cuenta con Retell propio. Solo la cuenta administradora lo ve.
function ReporteRetellGlobal() {
  ...
  return (
    <div className="mt-8 rounded-2xl border border-[var(--color-borde)] bg-[var(--color-tarjeta)] p-5">
      <button onClick={() => setAbierto((v) => !v)} className="flex w-full items-center justify-between text-left">
        <div>
          <h2 className="text-base font-semibold text-[var(--color-texto)]">Reporte completo de Retell</h2>
